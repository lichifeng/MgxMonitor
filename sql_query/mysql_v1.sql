-- 首页的统计数据
SELECT 'Unique game_guid count' AS description, COUNT(DISTINCT game_guid) AS count FROM games
UNION ALL
SELECT 'Unique player name count', COUNT(DISTINCT name) FROM players
UNION ALL
SELECT 'Games updated last month count', COUNT(*) FROM games WHERE MONTH(last_updated) = (MONTH(CURRENT_DATE()) - 1) AND YEAR(last_updated) = YEAR(CURRENT_DATE());
